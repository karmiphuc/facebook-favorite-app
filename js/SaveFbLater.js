  ...
  SaveFbLater = {
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (item != null && item != '') {
          ...
          try {
            ...
            var descText = descTable.text[item];
          }
    ...
    showFeed: function(timeHash) {
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (item != null && item != '') {
          var listEle = document.createElement('div');
          if (typeof descTable.isPublic != 'undefined' && typeof descTable.isPublic[item] != 'undefined' && (descTable.isPublic[item]) != 1 && typeof descTable.preview[item] != 'undefined') {
            ...
            try {
              ...
              var previewHtml = descTable.preview[item];
            }
    ...
    remove: function(itemToDel) {
      ...
      if (index > -1) {
        ...
        chrome.browserAction.setBadgeText({
          text: '' + this.myStorage.links.length
        });
        return true;
      } else return false;
    },
    ...
    populateNewTab: function(mode) {
      ...
      chrome.tabs.create({
        url: chrome.extension.getURL(mode + ".html"),
        active: true
      });
    },
